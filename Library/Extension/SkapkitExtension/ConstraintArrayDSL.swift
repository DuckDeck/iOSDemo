    ...
    public func distributeSudokuViews(fixedItemWidth: CGFloat,
        ...
        for (i,v) in self.array.enumerated() {
            ...
            v.snp.makeConstraints({ (make) in
                make.width.equalTo(fixedItemWidth)
                make.height.equalTo(fixedItemHeight)
                make.top.equalTo(padding.top + currentRow * (fixedItemHeight + verticalSpace))
                make.left.equalTo(padding.left + currentColumn * (fixedItemWidth + horizontalSpace))
            })
        }
        tempSuperView.snp.updateConstraints { (m) in
            let a = padding.top + padding.bottom
            let b = rowCount * fixedItemHeight + (rowCount - 1) * verticalSpace
            m.height.greaterThanOrEqualTo(a + b)
            
            let c = padding.left + padding.right
            let d = columnCount * fixedItemWidth + (columnCount - 1) * horizontalSpace
            m.width.greaterThanOrEqualTo(c + d)
        }
    }
