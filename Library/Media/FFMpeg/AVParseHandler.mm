...
- (void)startParseWithFormatContext:(AVFormatContext *)formatContext videoStreamIndex:(int)videoStreamIndex audioStreamIndex:(int)audioStreamIndex completionHandler:(void (^)(BOOL isVideoFrame, BOOL isFinish, struct ParseVideoDataInfo *videoInfo, struct ParseAudioDataInfo *audioInfo))handler{
    m_isStopParse = NO;
    ...
    dispatch_async(parseQueue, ^{
        ...
        while (!self->m_isStopParse) {
            ...
            if (packet.stream_index == audioStreamIndex) {
                ParseAudioDataInfo audioInfo = {0};
