...
- (void)startToAnalyzeImage{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
        ...
        unsigned char *rawData = [self rawPixelDataFromImage:self->_image];
        if (!rawData || self.pixelCount <= 0){
            ...
            NSError *nullImageError = [NSError errorWithDomain:NSCocoaErrorDomain code:NSFileNoSuchFileError userInfo:userInfo];
            self->_getColorBlock(nil,nil,nullImageError);
            return;
        }
        ...
        self->_distinctColors = [[NSMutableArray alloc]init];
        for (NSInteger color = 0; color < length ;color++){
            if (hist[color] > 0){
                [self->_distinctColors addObject: [NSNumber numberWithInteger:color]];
                distinctColorIndex++;
            }
        }
        ...
        if (distinctColorCount <= kMaxColorNum){
            ...
            for (NSInteger i = 0;i < distinctColorCount ; i++){
                NSInteger color = [self->_distinctColors[i] integerValue];
            ...
            self->_swatchArray = [swatchs copy];
        }else{
            self->_priorityArray = [[PriorityBoxArray alloc]init];
            VBox *colorVBox = [[VBox alloc]initWithLowerIndex:0 upperIndex:distinctColorIndex colorArray:self->_distinctColors];
            [self->_priorityArray addVBox:colorVBox];
            // split the VBox
            [self splitBoxes:self->_priorityArray];
            //Switch VBox to Swatch
            self.swatchArray = [self generateAverageColors:self->_priorityArray];
        }
...
//            colorModel.bodyTextColorString = [swatch getBodyTextColorString];
            ...
            if (!recommendColorModel){
                ...
                if (!_isNeedColorDic){
                    dispatch_async(dispatch_get_main_queue(), ^{
                        self->_getColorBlock(recommendColorModel,nil,nil);
                    });
                    return;
                }
            }
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        self->_getColorBlock(recommendColorModel,finalColorDic,nil);
    });
