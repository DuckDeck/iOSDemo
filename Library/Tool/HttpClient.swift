...
class HttpClient {
    ...
    fileprivate static let queue = DispatchQueue.init(label: "HttpClient")
    ...
    fileprivate var progress: ((_ progress: Float)->Void)?
    ...
    func useHandle(usehandle: Bool)->HttpClient {
        isUseHandle = usehandle
        return self
    }
    
    open func addParams(_ params: [String: Any]?)->HttpClient {
        self.params = params
        return self
    }
    
    open func addUrlParams(_ params: [String: Any]?)->HttpClient {
        urlPara = params
        return self
    }
    
    open func addHeaders(_ params: [String: String]?)->HttpClient {
        if params == nil {
            return self
        }
    ...
    open func addMultiParams(params: [String: Any]?)->HttpClient {
    ...
    open func completion(_ completion: ((_ data: Data?, _ error: Error?)->Void)?) {
        ...
        if isUseHandle {
            url = HttpClient.urlHandler?(url)
            if headers != nil {
                headers = HttpClient.httpHeaderHandler?(headers!)
            }
            if params != nil {
                params = HttpClient.paramsHandler?(params!)
            }
        }
        
        if multipartFormData != nil {
            AF.upload(multipartFormData: multipartFormData!, to: url, headers: headers).response { data in
                if let d = data.data {
    ...
    static func combineRequest(requests:[HttpClient],completed:@escaping (([Int:(String,Data?,Swift.Error?)])->Void)){
        var result = [Int:(String,Data?,Swift.Error?)]()
        let group = DispatchGroup.init()
        for sender in requests.enumerated() {
            group.enter()
            queue.async {
                sender.element.completion { data, err in
                    if data != nil{
                        if let s = String(data: data!, encoding: String.Encoding.utf8){
                            print("网络请求\(sender.element.url ?? "")已返回")
                            print(s)
                        }
                    }else{
                       
                    }
                    result[sender.offset] = (sender.element.url, data,err)
                    group.leave()
                }
            }
        }
    ...
    static func combineRequestWithDelay(requests:[HttpClient],completed:@escaping (([Int:(String,Data?,Swift.Error?)])->Void)){
        var result = [Int:(String,Data?,Swift.Error?)]()
        let group = DispatchGroup.init()
        if #available(iOS 10.0, *) {
            ...
            let timer = Timer.scheduledTimer(withTimeInterval: 0.1, repeats: true) {t in
                    ...
                    queue.async {
                        if(i >= requests.count){
                            ...
                            let sender = requests[i]
                            sender.index = i
                            sender.completion { data, err in
                            if data != nil{
                                if let s = String(data: data!, encoding: String.Encoding.utf8){
                                    print("网络请求\(sender.url ?? "")已返回")
                                    print(s)
                                }
                            }else{
                            }
                            result[sender.index] =  (sender.url, data,err)
                            group.leave()
                        }
                        i += 1
                    }
            ...
            HttpClient.combineRequest(requests: requests) { dict in
                completed(dict)
            }
        }
    }
    ...
    public struct ArrayEncoding: ParameterEncoding {
        ...
        public func encode(_ urlRequest: URLRequestConvertible, with parameters: Parameters?) throws->URLRequest {
            ...
            guard let parameters = parameters,
                  let array = parameters[arrayParametersKey]
            else {
                return urlRequest
            }
            do {
